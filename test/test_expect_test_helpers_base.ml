...
let%expect_test "[hide_paths_in_string]" =
  let test s = print_endline (hide_paths_in_string s) in
  test "lib/core/test/test_error.ml:17:3";
  [%expect {| test_error.ml:17:3 |}];
  test "lib/expect_test_helpers/base/test/test_expect_test_helpers_base.ml:LINE:COL";
  [%expect {| test_expect_test_helpers_base.ml:LINE:COL |}];
  test "no/path.ml here but also plain-text";
  [%expect {| path.ml here but also plain-text |}];
  test "just_a_filename.ml:10:5";
  [%expect {| just_a_filename.ml:10:5 |}];
  test "/absolute/path/to/file.mli";
  [%expect {| file.mli |}];
  test "multiple lib/a/foo.ml and lib/b/bar.ml paths";
  [%expect {| multiple foo.ml and bar.ml paths |}]
;;

let%expect_test "[print_s ~hide_paths:true]" =
  print_s ~hide_paths:true ~hide_positions:true [%message [%here] [%here]];
  [%expect
    {|
...
let%expect_test "[show_raise ~hide_paths:true]" =
  show_raise ~hide_paths:true ~hide_positions:true (fun () -> raise_s [%message [%here]]);
  [%expect
    {|
    (raised
     test_expect_test_helpers_base.ml:LINE:COL)
...
let%expect_test "[require_does_raise ~hide_paths:true] success" =
