...
module type Expect_test_helpers_base = sig @@ portable
  ...
  (** [hide_paths_in_string] strips directory prefixes from source file paths, preserving
      only the filename. For example, [lib/core/test/test_error.ml:17:3] becomes
      [test_error.ml:17:3]. This can be useful in making displayed test output less
      fragile when file locations move between directories. *)
  val hide_paths_in_string : string -> string
  ...
  (** Renders an s-expression as a string. With [~hide_positions:true], patterns in the
      ...
      column number, and character positions, to make output less fragile. With
      [~hide_paths:true], directory prefixes are stripped from file paths, preserving only
      the filename. Both options are composable. *)
  val sexp_to_string
  ...
  (** For printing an s-expression to stdout. [hide_positions] and [hide_paths] work as in
      [sexp_to_string]. *)
  val print_s
    :  ?hide_positions:bool (** default is [false] *)
    -> ?hide_paths:bool (** default is [false] *)
